...
int particion(vector<int>& vec, int bajo, int alto){
	int pivote = vec[alto/2];
	int ind = bajo - 1;
	...
	swap(vec[ind + 1], vec[alto]);
	return ind + 1;
}

void quickSort(vector<int>& vec, int bajo, int alto) {
	if (bajo < alto) {
        if (vec.size() <= 0)
        {
            return;
		...
		int pivot = particion(vec, bajo, alto);
		
		quickSort(vec, bajo, pivot - 1);
		quickSort(vec, pivot + 1, alto);
        }
    }
}
